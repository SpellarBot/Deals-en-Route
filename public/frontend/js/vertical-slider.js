;(function( $ ) {
    ...
    $.verticalSlider = function( sectionsContainer, options ) {
        ...
        var _construct = function() {
            ...
            Plugin.sections            = Plugin.sectionsContainer.children();
            ...
            Plugin.mobileDisabled      = false;
            
                
            Plugin.sectionsContainer.addClass("vs-slider");
            Plugin.sections.addClass("vs-section");
        ...
        Plugin.disableHammer = function(disable){
            if(hammerVS) hammerVS.set({ enable: !disable });  
        }
        ...
        var _bindEvents = function() {
            ...
            $( window ).on('resize', function() {
                ...
                if($( window ).width()<1024){
                    Plugin.mobileView = true;
                    _resetSlider();
                }else{
                    Plugin.mobileView = false;
                }
            });

            // Scroll with mousewheel actions
            Plugin.sectionsContainer.mousewheel(function( event ) {
                // Check the scroll direction and move in consequence
                if(!Plugin.mobileView){
            ...
            $( window ).on('keyup', function( event ) {
                if( event.which === 40 && !Plugin.mobileView ) {
                    Plugin.next();
                } else if( event.which === 38 && !Plugin.mobileView ) {
                    Plugin.prev();
                }
            });
        ...
        var _resetSlider = function() {
            isConstructed = false;
            Plugin.sectionsContainer.removeClass("vs-slider").removeAttr("style");
            Plugin.sections.removeClass("vs-section").removeAttr("style");
        }
        ...
        $( window ).on('resize', function() {
            if($( window ).width()>=1024 && !isConstructed){
                _construct();
            }
        });
